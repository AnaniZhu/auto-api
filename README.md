...
默认值分别任 `./src/interface`, `./src/request`
...
- 当为 object 时，它有 4 个属性，分别为 `hash`, `name`, `includes`, `excludes`。`hash` 代表模块id，name 代表生成文件的文件名[name]的占位符，`includes` 和 `excludes` 皆为一个数组，数组每一项都是字符串，代表单个接口的id，先会导入 `incldues` 的所有接口，再经过 excludes 过滤，过滤后的接口才会进行处理(生成 interface 和 request)。
下面看一个例子:
```
